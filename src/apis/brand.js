...
export const getProductBrand =  () =>{
    ...
    const {type,brand} = state
    const {page,limit} = state.pagination
    const url = `http://localhost:3000/products?category=${idCategory}${idDetailCategory ? `&detail_category=${idDetailCategory}`:``}${handleTypeChecked(type) ? handleTypeChecked(type) : ``}${brand ? handleBrandChecked(brand) :`` }&_page=${page}&_limit=${limit}`;
    const option = {
        method : 'GET',
        mode : 'cors',
        headers: {
