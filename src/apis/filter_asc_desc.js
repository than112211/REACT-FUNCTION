...
export const getProductDesc=  () =>{
    ...
    const {type,brand,rating,search} = state
    const {page,limit} = state.pagination
    const url = `http://localhost:3000/products?category=${idCategory}${idDetailCategory ? `&detail_category=${idDetailCategory}`:``}${handleTypeChecked(type) ? handleTypeChecked(type) : ``}${handleBrandChecked(brand) ? handleBrandChecked(brand) : ``}${rating ? `&rating=${rating}` :``}${ search ? `&name_like=${search}`:``}&_sort=price&_order=desc&_page=${page}&_limit=${limit}`
    const option = {
        method : 'GET',
        mode : 'cors',
        headers: {
